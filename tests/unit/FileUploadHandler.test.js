...
import { createTestFile, cleanupTestFiles } from '../setup.js';
...
test.test('extractFromTextFile should read text content', async () => {
  const handler = new FileUploadHandler();
  const filePath = await createTestFile('test.txt', 'Hello, world!');
  const result = await handler.extractFromTextFile(filePath);
  
  assert.equal(result, 'Hello, world!');
});

test.test('extractFromTextFile should throw error for non-existent file', async () => {
  const handler = new FileUploadHandler();
  
  await assert.throwsAsync(
    () => handler.extractFromTextFile('non-existent.txt'),
